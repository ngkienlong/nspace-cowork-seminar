...
1. **SlidePresentation Class** — Main controller with:
   ...
   - Numbered dot navigation (mandatory, see below)
...
5. **Numbered Dot Navigation (MANDATORY)** — Always include in every presentation:
   - A fixed `<nav>` bar at the bottom center of the viewport with one dot per slide
   - Each dot displays its slide number (1, 2, 3...)
   - Clicking a dot jumps directly to that slide
   - The active slide's dot is visually highlighted (larger, accent color)
   - Dots are generated dynamically via JS from the slide count
   - Style the dots to match the deck's theme (use accent color for active state)
   ...
   CSS:
   ```css
   .dot-nav { position:fixed; bottom:22px; left:50%; transform:translateX(-50%); z-index:9998; display:flex; gap:8px; align-items:center; padding:10px 18px; background:rgba(0,0,0,0.6); border-radius:20px; backdrop-filter:blur(8px); }
   .dot-nav .dot { width:22px; height:22px; border-radius:50%; background:var(--text-secondary, #666); opacity:0.4; cursor:pointer; transition:all 0.3s ease; flex-shrink:0; display:flex; align-items:center; justify-content:center; font-family:var(--font-body, monospace); font-size:9px; font-weight:500; color:var(--bg-primary, #000); line-height:1; }
   .dot-nav .dot:hover { opacity:0.7; transform:scale(1.2); }
   .dot-nav .dot.active { background:var(--accent, #F5A623); opacity:1; transform:scale(1.2); }
   ```

   JS (inside SlidePresentation class):
   ```javascript
   setupDotNav() {
   ...
   // In showSlide(), sync dots:
   // if (this.dots) this.dots.forEach((d, i) => d.classList.toggle('active', i === this.currentSlide));
   ```
